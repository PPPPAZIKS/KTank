...
完整玩法、房间、胜负及重连规则见 [`docs/GAME_RULES.md`](docs/GAME_RULES.md)。
...
5. 开局后仍可加入房间，但新玩家本局作为已淘汰玩家等待下一局。
...
- 鼠标：瞄准
- 鼠标点击或空格：射击
- “退出房间”：返回大厅，原玩家状态保留 30 秒，重新加入同一房间可继续游戏
- 主动退出或网络中断时，客户端均可在 30 秒保留期内恢复原玩家和对局
...
4. 新客户端加入时立即获得房间完整状态。
5. 客户端主动退出或意外断线后会标记为重连中，并可在 30 秒内恢复原玩家；超时后才移除。
6. 无效房间号、满员房间、连接失败和无权限操作会显示明确提示。
...
客户端只发送 `joinRoom`、`playerInput`、`fire`、`startGame`、`restart`、`leaveRoom` 操作意图。服务端以 30 Tick 更新位置、子弹、碰撞、生命值和胜负，以 20Hz 向房间广播完整 `snapshot`。玩家使用会话内稳定 `sessionId` 标识，意外断线后保留 30 秒以支持自动重连。
